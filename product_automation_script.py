...
def list_designs_from_gcs() -> List[dict]:
    """List all PNG files from GCS bucket and create rows for processing."""
    ...
    for blob in blobs:
        ...
        # Normalize: remove trailing _N or _scale to group variations
        # e.g., "blue_polkadot_pattern_1.png" → "blue_polkadot_pattern"
        import re
        base_name = re.sub(r'(_scale|_\d+)?\.png$', '', filename)
        
        # Keep only the first occurrence (oldest) of each design name
        if base_name not in seen_filenames:
            seen_filenames[base_name] = blob.name
    
    # Create rows from deduplicated designs
    rows = []
    for idx, (base_name, full_path) in enumerate(sorted(seen_filenames.items()), start=1):
        sku = str(idx)
        
        rows.append({
            "SKU #": sku,
            "SKU": sku,
            "Image Path": f"gs://{GCS_BUCKET}/{full_path}",
            "Product Name": "",  # Will be generated by Gemini
            "Design Theme": "",
            "Primary Colors": "",
            "Keywords / Tags": "",
        })
...
4. Description should emphasize benefits over specs
...
def step2_generate_printify_mockups(rows: List[dict], state: dict, single_sku: str = None):
    ...
    for row in rows:
        ...
        try:
            design_url = upload_to_gcs(local_path, f"designs/step2-uploads/{design_name}.png")
        except Exception as exc:
...
def main():
    ...
    state = load_state()
    log.info("✓ Discovered %d design files from GCS", len(rows))
